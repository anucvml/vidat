  ...
  <div>
    ...
    <div style="position: relative;">
      ...
      <div
        ...
        {{ action.text }}<span v-if="actionList.length !== 1 && action.id !== actionList.length - 1"
      ...
      <div v-show="popup.x" v-if="annotationList"
           :style="{position: 'absolute', top: popup.y + 'px', left: popup.x + 'px'}">
        <q-popup-edit ref="popup" :value="1" :title="'new ' + mode" @show="handlePopupShow">
          ...
          ></q-input>
          <q-btn dense falt class="full-width q-mt-sm" @click="$refs.popup.hide()" color="primary"
                 label="confirm"></q-btn>
        </q-popup-edit>
      </div>
    </div>
...
export default {
        ...
        for (let regionAnnotation of this.annotationList) {
          if (regionAnnotation.highlight) {
            ...
            if (this.backspaceDown && nearPoint) {
              ...
              const pointIndexList = regionAnnotation.getPointIndexListOfBoundary(mouseX, mouseY)
    ...
    draw () {
      if (this.preference.objects) {
        if (!this.objectAnnotationList) {
          ...
          for (const annotation of this.objectAnnotationList) {
      ...
      if (this.preference.regions) {
        if (!this.regionAnnotationList) {
          ...
          for (const annotation of this.regionAnnotationList) {
            annotation.draw(this.ctx)
          }
        }
      }
      if (this.preference.skeletons) {
        if (!this.skeletonAnnotationList) {
          ...
          for (const annotation of this.skeletonAnnotationList) {
            annotation.draw(this.ctx)
          }
        }
      }
  ...
  computed: {
    ...
    labelOption () {
      ...
      const labelOption = []
      for (const objectLabel of objectLabelData) {
        labelOption.push({
