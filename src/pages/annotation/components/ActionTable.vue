...
import { ActionAnnotation } from 'src/libs/annotationlib.js'
...
export default {
  ...
  methods: {
    ...
    handleDelete (row) {
      utils.confirm('Are you sure to delete this action?').onOk(() => {
        for (const i in this.actionAnnotationList) {
          if (this.actionAnnotationList[i] === row) {
  ...
  computed: {
    ...
    actionOptionList () {
      ...
      for (const actionLabel of actionLabelData) {
    ...
    objectOptionMap () {
      ...
      for (const action of this.actionLabelData) {
        ret[action.id] = []
        for (const object of this.objectLabelData) {
