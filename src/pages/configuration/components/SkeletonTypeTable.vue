...
import SkeletonTypePreview from './SkeletonTypePreview'
...
export default {
  ...
  methods: {
    ...
    handleAdd () {
      const lastId = this.tableData.length ? this.tableData[this.tableData.length - 1].id : -1
      this.tableData.push({
        id: lastId + 1,
        name: 'new',
        description: '',
    ...
    handleAddPoint (props) {
      const pointList = props.row.pointList
      const lastId = pointList.length ? pointList[pointList.length - 1].id : -1
      pointList.push({
        id: lastId + 1,
        name: 'new',
        x: 0,
    ...
    handleAddEdge (props) {
      const edgeList = props.row.edgeList
      const lastId = edgeList.length ? edgeList[edgeList.length - 1].id : -1
      edgeList.push({
        id: lastId + 1,
        from: -1,
        to: -1,
