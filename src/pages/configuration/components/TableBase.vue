<template>
  ...
  >
    ...
    <template v-slot:body="props">
      <q-tr
        :class="{
          'bg-green-2': configurationStore.currentThumbnailActionLabelId === props.row.id && title === 'Action Labels'
        }"
      >
        <q-td
          v-if="expand"
...
const handleThumbnailPreview = (row) => {
  configurationStore.currentThumbnailActionLabelId =
    configurationStore.currentThumbnailActionLabelId === row.id ? null : row.id
}
